...
//             currentCamera->move( deltaTime, FORWARD );
    ...
    void Looper::renderParticles( unsigned depthId ) {
...
//            TODO - use depth buffer to render particles spherically
//            opacity of a particle should decrease gradually if there is an object in front of it
//            - the depth buffer here is empty
...
//        glEnable(GL_DEPTH_TEST);
    ...
    void Looper::render() {
        ...
        renderScene( shader );
        unsigned depthId = copyDepthBuffer();
        skyboxShader.use();
...
//         entities["building1"].scale( { 1, 1, 1 } );
    ...
    unsigned Looper::copyDepthBuffer() {
        unsigned id;
        glGenTextures(1, &id);
        glBindTexture(GL_TEXTURE_2D, id);
        glReadBuffer(GL_BACK);
        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, 0, 0, window.width, window.height, 0 );
        return id;
    }
}
